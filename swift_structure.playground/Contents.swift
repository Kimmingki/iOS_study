...
struct Store {
    ...
    func isDeliverale(userLoc: Location) -> Bool {
...
func printClosestStore(currentLocation: Location, stores: [Store]) {
    ...
    for store in stores {
        ...
        if closestStoreDistance == distanceToStore {
            ...
            isDelivarable = store.isDeliverale(userLoc: currentLocation)
        }
    }
    
    print("Closest store: \(closestStoreName), isDeliverable: \(isDelivarable)")
}
